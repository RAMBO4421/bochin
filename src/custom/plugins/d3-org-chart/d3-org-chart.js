function Chart() {
    ...
    var main = function() {
        ...
        layouts.treemap = d3.tree().size([calc.chartWidth, calc.chartHeight]).nodeSize([calc.nodeMaxWidth + 100, calc.nodeMaxHeight + attrs.depth]);
 ...
 `;
        ...
        function update(source) {
            ...
            // Styling links
            linkUpdate.attr('fill', 'none').attr('stroke-width', d => d.data.connectorLineWidth || 2).attr('stroke', d => {
            ...
            nodeEnter.patternify({
                ...
                selector: 'node-rect',
                data: d => [d],
            }).attr('width', 1e-6).attr('height', 1e-6).style('fill', function(d) {
                return d._children ? 'lightsteelblue' : '#fff';
            });

            // Add foreignObject element
            const fo = nodeEnter.patternify({
                tag: 'foreignObject',
                selector: 'node-foreign-object',
                data: d => [d],
            }).attr('width', d => d.width).attr('height', d => d.height).attr('x', d => -d.width / 2).attr('y', d => -d.height / 2);
            ...
            fo.patternify({
                ...
                data: d => [d],
            }).style('width', d => d.width + 'px').style('height', d => d.height + 'px').style('color', 'white').html(d => d.data.template);
            ...
            nodeEnter.patternify({
                ...
                data: d => [d],
            }).text('test').attr('x', d => -d.width / 2 + 7).attr('y', d => d.height / 2 - d.data.nodeIcon.size - 5)
                //.attr('text-anchor','middle')
                .text(d => d.data.totalSubordinates + ' Subordinates').attr('fill', attrs.nodeTextFill).attr('font-weight', 'bold');
            ...
            nodeUpdate.transition().attr('opacity', 0).duration(attrs.duration).attr('transform', function(d) {
            ...
            nodeUpdate.select('.node-button-g').attr('transform', d => {
