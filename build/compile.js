            ...
            if ('required' in bundle.src && 'optional' in bundle.src) {
                ...
                for (var vendor in vendors) {
                    ...
                    for (var type in vendorObj) {
                        ...
                        switch (type) {
                            case 'fonts':
                                gulp.src(vendorObj[type]).pipe(_self.outputStream(bundle.bundle.fonts)());
                                break;
                            case 'images':
                                gulp.src(vendorObj[type]).pipe(_self.outputStream(bundle.bundle.images)());
                                break;
                            case 'json':
                                gulp.src(vendorObj[type]).pipe(_self.outputStream(bundle.bundle.json)());
                                break;
                            case 'media':
                                gulp.src(vendorObj[type]).pipe(_self.outputStream(bundle.bundle.media)());
                                break;
                        }
                    }
                }
    ...
    output: function(bundle) {
        ...
        if (typeof bundle.src !== 'undefined' && typeof bundle.output !== 'undefined') {
            for (var type in bundle.src) {
                ...
                switch (type) {
                    case 'styles':
                        gulp.src(bundle.src[type]).pipe(sassGlob()).pipe(sass({
                            errLogToConsole: true,
                        }).on('error', sass.logError)).pipe(_self.outputStream(bundle.output[type])());
                        break;
                    default:
                        gulp.src(bundle.src[type]).pipe(_self.outputStream(bundle.output[type])());
                        break;
                }
            }
        }
        ...
        menu.forEach(function(group, index) {
            ...
            if (group.items) {
                ...
                group.items.forEach(function(item) {
                    ...
                    lines.push('<a path=' + item.href.replace(/_/g, '') +' href="'.concat(item.href, '" title="').concat(item.title, '" data-filter-tags="').concat(item.tags, '">'));
                    ...
                    if (item.items) {
                        ...
                        item.items.forEach(function(subItem) {
                            ...
                            lines.push('<a href="'.concat(subItem.href, '" title="').
                                concat(subItem.title, '" data-filter-tags="').
                                concat(subItem.tags, '">'));
                ...
                lines.push('<li class="'+ classItem +'" href=' + group.href.replace(/_/g, '') + ' {{#is pagename "in" "'.concat(groups, '"}} class="active open"{{/is}}>'));
                lines.push('<a href="'.concat(group.href, '" title="').concat(group.title, '" data-filter-tags="').concat(group.tags, '">'));
                lines.push('<i class="'.concat(group.icon, '"></i>'));
                lines.push('<span class="nav-link-text" data-i18n="'.concat(group.i18n, '">').concat(group.text, '</span>'));
                if (group.span) {
                    group.span.class = group.span.class || '';
